...
-- 目的港费用表
CREATE TABLE IF NOT EXISTS `dest_port_charge` (
    `id`               BIGINT        NOT NULL AUTO_INCREMENT,
    `port_name`        VARCHAR(200)  NOT NULL COMMENT '目的港名称（原始）',
    `port_name_clean`  VARCHAR(200)  NOT NULL COMMENT '目的港名称（清洗后，用于匹配）',
    `sheet_name`       VARCHAR(50)   COMMENT '来源Sheet',
    `item_cn`          VARCHAR(100)  COMMENT '中文费用名称',
    `item_en`          VARCHAR(100)  COMMENT '英文费用代码',
    `currency`         VARCHAR(10)   COMMENT '货币（USD/EUR/HKD/THB等）',
    `direct_rate_str`  VARCHAR(100)  COMMENT '直客费率（原始字符串）',
    `direct_rate_num`  DECIMAL(12,4) COMMENT '直客费率（数值，用于计算）',
    `direct_unit`      VARCHAR(20)   COMMENT '直客计费单位(BL/WM/RT/TON/SET)',
    `coloader_rate_str` VARCHAR(100) COMMENT '同行费率（原始字符串）',
    `coloader_rate_num` DECIMAL(12,4) COMMENT '同行费率（数值）',
    `coloader_unit`    VARCHAR(20)   COMMENT '同行计费单位',
    `min_charge`       DECIMAL(12,2) COMMENT '最低收费',
    `remarks`          VARCHAR(500)  COMMENT '备注',
    `deleted`          TINYINT       NOT NULL DEFAULT 0,
    `create_time`      DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `update_time`      DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    KEY `idx_port` (`port_name_clean`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='目的港费用标准表';

-- 目的港费用上传日志
CREATE TABLE IF NOT EXISTS `dest_upload_log` (
    `id`          BIGINT       NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `dest_port_charge` (
    `id`                 BIGINT        NOT NULL AUTO_INCREMENT,
    `source_sheet`       VARCHAR(50)   NOT NULL COMMENT '来源Sheet',
    `destination`        VARCHAR(200)  NOT NULL COMMENT '目的港名称',
    `fee_name_cn`        VARCHAR(100)  DEFAULT NULL COMMENT '中文费项',
    `fee_name_en`        VARCHAR(100)  DEFAULT NULL COMMENT '英文费项',
    `currency`           VARCHAR(10)   DEFAULT NULL COMMENT '货币',
    `amount_direct_raw`  VARCHAR(100)  DEFAULT NULL COMMENT '直客原始金额字符串',
    `amount_direct`      DECIMAL(12,4) DEFAULT NULL COMMENT '直客金额(数值)',
    `unit_direct`        VARCHAR(20)   DEFAULT NULL COMMENT '直客计费单位: BL/WM/SET/TON/BLOCK',
    `amount_coload_raw`  VARCHAR(100)  DEFAULT NULL COMMENT '同行原始金额',
    `amount_coload`      DECIMAL(12,4) DEFAULT NULL COMMENT '同行金额(数值)',
    `unit_coload`        VARCHAR(20)   DEFAULT NULL COMMENT '同行计费单位',
    `min_direct`         DECIMAL(12,4) DEFAULT NULL COMMENT '直客最低消费',
    ...
    PRIMARY KEY (`id`),
    KEY `idx_destination` (`destination`),
    KEY `idx_sheet` (`source_sheet`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='目的港费用明细';

-- 目的港费用上传日志
CREATE TABLE IF NOT EXISTS `port_charge_upload_log` (
    `id`          BIGINT       NOT NULL AUTO_INCREMENT,
    `file_name`   VARCHAR(255) NOT NULL,
    `upload_date` DATE         DEFAULT NULL,
    `total`       INT          DEFAULT 0,
    ...
    `create_time` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='目的港费用上传日志';
